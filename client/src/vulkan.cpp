...
#define VK_CHECK(expr) \
   do { \
      ...
      if (__result != VK_SUCCESS) \
         std::cout << "'" << #expr << "' line " << __LINE__ << " failed with " << vk_Result_to_str(__result) << std::endl; \
   } while (0)
                                                 ...
                                                 struct device_data *device_data)
{
   ...
   data->window_size = ImVec2(200, 200);
...
static void create_fonts(ImFontAtlas* font_atlas, ImFont*& small_font, ImFont*& text_font)
{
   auto& io = ImGui::GetIO();
   if (!font_atlas)
        font_atlas = io.Fonts;
   ...
   ImGui::GetIO().FontGlobalScale = 1.f; // set here too so ImGui::CalcTextSize is correct
   float font_size = 24;
                                           ...
                                           unsigned imageIndex)
{
   ...
   swapchain_data->sw_stats.last_present_time = 0;
   swapchain_data->sw_stats.n_frames++;
   swapchain_data->sw_stats.n_frames_since_update++;
    ...
    VkSwapchainKHR*                             pSwapchain)
{
   ...
   if (fpGetPhysicalDeviceSurfacePresentModesKHR != NULL) {
      ...
      fpGetPhysicalDeviceSurfacePresentModesKHR(device_data->physical_device, pCreateInfo->surface, &presentModeCount, presentModes.data());
   }
    ...
    VkInstance*                                 pInstance)
{
   ...
   _params->font_params_hash = get_hash(_params->font_file);
